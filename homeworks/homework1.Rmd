...
Since the alpha and beta values are 
```{r lab1}
library(AER)
data("ShipAccidents")
...
h1 = c(1,1,1,200,.5)  
h2 = c(0.5,2,10000,2,0) 

montlyIncidentsCountNormalized=cleanData$norm_incidents/min_service*100
...
(h1+sum(montlyIncidentsCountNormalized))/(h2+incidentsCount)
##MAP
(h1+sum(montlyIncidentsCountNormalized)-1)/(h2+incidentsCount)
```
...
```{r}
alpha=0.05
qgamma(c(alpha/2,1-alpha/2),sum(montlyIncidentsCountNormalized) + 1/2 ,incidentsCount)
```
...
hpd(rgamma(1000,.5+sum(montlyIncidentsCountNormalized),incidentsCount),.95)
```
