//add exposure
//fit in cui vario l'intercetta sulle nazioni
functions {
...
model {
  deaths ~ neg_binomial_2_log(mu[group_idx] + beta * uvb + population , phi);
...
generated quantities {
  ...
  vector[N] log_lik;
  for (n in 1:N) {
    real eta_n = mu[group_idx[n]] + beta * uvb[n]+ population[n];
