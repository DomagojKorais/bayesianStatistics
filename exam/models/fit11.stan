...
data {
  ...
  int<lower=1> J;//numero di REGIONI
   int<lower=1> L;//numero di nazioni
  int<lower=0> region_idx[N];
  int<lower=0> nation_idx[N];
  //int<lower=1, upper=J> region_idx[N];//indice della nazione
...
model {
  deaths ~ neg_binomial_2_log(mu[nation_idx[region_idx]] + beta * uvb + population , phi);
...
generated quantities {
  ...
  vector[N] log_lik;
  for (n in 1:N) {
    real eta_n = mu[nation_idx[region_idx[n]]] + beta * uvb[n]+ population[n];
