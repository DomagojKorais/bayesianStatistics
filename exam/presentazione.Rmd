...
print(fit9, pars = c('sigma_mu','beta','alpha','phi','mu'))
y_rep <- as.matrix(fit9, pars = "y_rep")

ppc_dens_overlay(data_complete2$deaths, y_rep[1:284,])+
...
#std residuals
mean_inv_phi <- mean(as.matrix(fit9, pars = "inv_phi"))
...
ggplot(plotData,aes(x=mean_y_rep,y=std_resid))+
  ...
  geom_hline(yintercept = -3,color = "green")+
  ggtitle("Std residuals for fit 9")

posterior <- as.array(fit9)
dim(posterior)
mcmc_areas(posterior,regex_pars = "mu")
